...
class DeclareOrDefineVisitor : public Visitor {
  ...
  // Define struct TypeInfos in each referencing CU.
  void visit(TypeInfoStructDeclaration *decl) override {
    ...
    // The special members might still need sema3, to determine whether they are
    // compilable. E.g., the following call might set `sd->xeq` to
