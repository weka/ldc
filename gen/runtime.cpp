...
llvm::GlobalVariable *LLVM_D_GetRuntimeGlobal(Loc &loc, llvm::Module &target,
                                              const char *name) {
...
static LLType *rt_ptr(LLType *t) { return getPtrToType(t); }
...
static LLType *rt_dg1() {
  ...
  LLFunctionType *fty =
      LLFunctionType::get(LLType::getInt32Ty(gIR->context()), types1, false);

  LLType *types[] = {rt_ptr(LLType::getInt8Ty(gIR->context())), rt_ptr(fty)};
  return LLStructType::get(gIR->context(), types, false);
}
...
static void LLVM_D_BuildRuntimeModule() {
  ...
  // bool _d_decide_maybe()
  {
    llvm::StringRef fname("_d_decide_maybe");
    LLFunctionType *fty = llvm::FunctionType::get(boolTy, false);
...
#define STR_APPLY1(TY, a, b)                                                   \
  {                                                                            \
    ...
    llvm::Function::Create(fty, llvm::GlobalValue::ExternalLinkage, fname1,    \
                           M);                                                 \
    llvm::Function::Create(fty, llvm::GlobalValue::ExternalLinkage, fname2,    \
                           M);                                                 \
    llvm::Function::Create(fty, llvm::GlobalValue::ExternalLinkage, fname3,    \
                           M);                                                 \
    llvm::Function::Create(fty, llvm::GlobalValue::ExternalLinkage, fname4,    \
                           M);                                                 \
  }
...
#undef STR_APPLY2
  ...
  // void* _aaGetY(AA* aa, const TypeInfo aati, in size_t valuesize, in void*
  // pkey)
  {
  ...
  // inout(void[]) _aaValues(inout AA aa, in size_t keysize, in size_t
  // valuesize, const TypeInfo tiValueArray)
  {
  ...
  // inout(void[]) _aaKeys(inout AA aa, in size_t keysize, const TypeInfo
  // tiKeyArray)
  {
  ...
  // AA _d_assocarrayliteralTX(const TypeInfo_AssociativeArray ti, void[] keys,
  // void[] values)
  {
  ...
  {
    // KLUDGE: _d_invariant is actually extern(D) in the upstream runtime,
    // possibly
    ...
    llvm::Function *fn = llvm::Function::Create(
        ...
        gABI->mangleForLLVM("_D9invariant12_d_invariantFC6ObjectZv", LINKd), M);
    assert(dty->ctype);
    IrFuncTy &irFty = dty->ctype->getIrFuncTy();
    gABI->rewriteFunctionType(dty, irFty);
    fn->addAttributes(
        1, llvm::AttributeSet::get(gIR->context(), 1, irFty.args[0]->attrs));
  ...
  if (global.params.isLinux) {
    ...
    llvm::StructType *dsoDataTy =
        llvm::StructType::get(sizeTy,                  // version
  ...
  if (global.params.cov) {
    ...
    LLType *params[] = {stringTy, rt_array(sizeTy), rt_array(intTy), byteTy};

    LLFunctionType *fty = LLFunctionType::get(voidTy, params, false);
    llvm::Function *fn =
        LLFunction::Create(fty, LLGlobalValue::ExternalLinkage, fname, M);
